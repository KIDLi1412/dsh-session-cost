...
function resolvePricing(rawConfig) {
	...
	for (const [key, entry] of Object.entries(overrides)) {
		...
		const base = merged[key] ?? { input: 0, cacheRead: 0, cacheWrite: 0, output: 0 };
		const numeric = (value, fallback) => typeof value === "number" && Number.isFinite(value) && value >= 0 ? value : fallback;
		merged[key] = {
			input: numeric(entry.input, base.input),
			cacheRead: numeric(entry.cacheRead, base.cacheRead),
			cacheWrite: numeric(entry.cacheWrite, base.cacheWrite),
			output: numeric(entry.output, base.output)
		};
	}
	return merged;
}
...
/**
 * Plugin body: register the summary and balance routes.
...
export { apply, inject, name, SUMMARY_PATH, BALANCE_PATH, foldSessionCost, resolvePricing };
