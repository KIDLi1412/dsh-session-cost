...
- `lib/client.js` — 浏览器 half：**手写 `window.__ModuleLoader__.load({id, factory})` bundle，无构建步骤**；React 组件（`require("react")` / `require("react/jsx-runtime")`）；CSS 走 `data-plugin-css` 通道（factory 内注入 `<style>`）；exports `{ inject, apply, ... }`
  ...
  - **不再注册 `settings.plugin.item`**（0.2.6 起）：低余额阈值只有一个编辑器，就在明细面板里——**「更新于 / ⟳」下面一行**（0.2.7 起），即 `div[data-slot=panel-setting]`：左侧标签 + 右侧 `<input data-slot="threshold-input">` + 单位。Host 侧仍注册 `session-cost` namespace、客户端仍 `ctx.settingsScope.bind({namespace:"session-cost"})`——**值照旧持久化在 settings.yaml**，搬的只是界面；别再往 DSH 设置里加卡片
    - 该行在 `panel-list`（读数网格）**之外**，也不含任何被 patch 的 slot（`updateMergeNode` 只按名字 patch 已知 slot），所以原地更新根本不碰它，30 秒自动刷新不会吃掉正在输入的值；提交走 `change`（失焦/回车）+ 显式 Enter，值经 `sanitizeThreshold` 写回 `configStore.set({lowBalanceThreshold})` → `scope.set`；空串/负数**不提交**并把输入框恢复成已存值；回调从 `data.onThreshold` 在**事件时**读取（同 ⟳ 按钮）
