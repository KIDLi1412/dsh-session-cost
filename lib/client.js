...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const tagId = "dsh-session-cost/SessionCost.module.css";
		...
		const zh = {
			...
			tooltipPricingNote: "费用为估算值：token 用量来自会话日志，单价见官方定价页（deepseek-v4-flash 输入 ¥1/百万、缓存命中 ¥0.02/百万、输出 ¥2/百万）。",
		...
		const en = {
			...
			tooltipPricingNote: "Cost is an estimate: token usage comes from the session log; unit prices follow the official pricing page (deepseek-v4-flash input ¥1/M, cache hit ¥0.02/M, output ¥2/M).",
		...
		//#region config store (localStorage)
		/**
		 * Display-mode + low-balance-threshold preference, persisted in
		 * localStorage (a UI choice, so it needs no server round-trip).
		 * `dock` = the standalone status bar below the stats line (the original
		 * layout, kept as the default); `stats` = cost/balance segments
		 * appended INTO the built-in stats line. `lowBalanceThreshold` (CNY,
		 * default 10) colors the balance red when it drops below the value.
		 */
		const CONFIG_KEY = "dsh-session-cost:config";
		...
		function readConfig() {
			try {
				...
				if (raw !== null) {
					...
					return {
						displayMode: parsed.displayMode === "stats" ? "stats" : "dock",
						lowBalanceThreshold: sanitizeThreshold(parsed.lowBalanceThreshold)
					};
				}
		...
		const configStore = {
			...
			set: (patch) => {
				const next = { ...configSnapshot, ...patch };
				if (next.displayMode !== "dock" && next.displayMode !== "stats") next.displayMode = "dock";
				next.lowBalanceThreshold = sanitizeThreshold(next.lowBalanceThreshold);
				configSnapshot = next;
				try {
					if (typeof localStorage !== "undefined") localStorage.setItem(CONFIG_KEY, JSON.stringify(configSnapshot));
				} catch { /* storage unavailable: keep the in-memory value */ }
				for (const listener of configListeners.slice()) listener();
			}
		};
		//#endregion
		...
		function buildMergeNode(dict, data) {
			...
			if (models.length === 0) {
				...
				for (const row of models) {
					...
					titleParts.push(`  ${label} · ${dict("tooltipInput")} ${fmt(row.inputTokens)} ${dict("tooltipTokens")} · ${dict("tooltipOutput")} ${fmt(row.outputTokens)} ${dict("tooltipTokens")} · ${dict("currencySymbol")}${fmtCny(row.cost, 4)}`);
				}
			}
		...
		function SessionCostBar({ useSession, useProjection, t, session, input }) {
			...
			// sessions show nothing until stats exist).
			react.useEffect(() => {
			...
			if (models.length === 0) {
				...
				for (const row of models) {
					const label = row.model.includes("/") ? row.model.slice(row.model.indexOf("/") + 1) : row.model;
					tooltipLines.push(`  ${label} · ${dict("tooltipInput")} ${fmt(row.inputTokens)} ${dict("tooltipTokens")} · ${dict("tooltipOutput")} ${fmt(row.outputTokens)} ${dict("tooltipTokens")} · ${dict("currencySymbol")}${fmtCny(row.cost, 4)}`);
				}
			}
		 ...
		 * the display-mode switch (独立状态栏 ↔ 并入统计栏), persisted to
