 ...
 * and 14:00–18:00 (Mon–Fri), and the off-peak rate is exactly half the peak
 * rate. Since 2026-08-23 00:00 Beijing time weekends (Sat–Sun) no longer
 * distinguish peak/off-peak and are billed at the off-peak rate all day.
...
export const DEFAULT_PRICING = {
	...
	// Vision variant of flash, priced identically (images bill as input
	// tokens); listed on the pricing page since V4-Flash-Vision-Exp.
	"deepseek-v4-flash-vision-exp": {
		offpeak: { input: 1.5, cacheRead: 0.05, cacheWrite: 1.5, output: 4.5 },
		peak: { input: 3, cacheRead: 0.1, cacheWrite: 3, output: 9 }
	},
	// DSH 0.1.5 routes the same V4-Flash model under the SHORT id
	// `deepseek-flash` (the composer labels it "DeepSeek-V41-Flash"). Without
	// this entry `priceOf` resolves no rate, every row prices at 0 and the
	// whole feature silently reports ¥0 — the 0.1.5 upgrade symptom.
	// `deepseek-v41-flash` is the same model under its other spelling. Both
	// bill at the V4-Flash rates above.
	"deepseek-flash": {
		offpeak: { input: 1.5, cacheRead: 0.05, cacheWrite: 1.5, output: 4.5 },
		peak: { input: 3, cacheRead: 0.1, cacheWrite: 3, output: 9 }
	},
 ...
 * `peak` during Beijing weekday 09:00–12:00 / 14:00–18:00 and `offpeak`
 * otherwise (since 2026-08-23 00:00 Beijing time, weekends Sat–Sun are
 * off-peak all day). A missing/invalid timestamp defaults to `offpeak` (the
 * majority regime).
 */
...
export function priceOf(modelKey, pricing) {
	...
	for (const key of Object.keys(table)) {
		...
		if (!modelId.startsWith(candidate)) continue;
		const boundary = modelId.charAt(candidate.length);
		if (boundary !== "" && !SEGMENT_SEPARATORS.has(boundary)) continue;
