...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// half (no dock entry, no merge node, no panel) rather than
		...
		const css = [
			...
			// The panel's one setting, on a line of its own UNDER the 更新于/⟳
			// row: label on the left, the field pushed to the same right edge the
			// reading values use, at the panel's 12/18 tier with DSH's own input
			// recipe (bg-base on a border-l2 stroke, rounded 6). The native
			// spinners are hidden — at 68px they would overlap the number — while
			// the field stays a number input, so ↑/↓ still steps it.
			".sco_mergePanelSetting{align-items:center;gap:8px;margin-top:6px;display:flex}",
			".sco_mergePanelSettingLabel{color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:18px}",
			".sco_mergeThresholdInput{box-sizing:border-box;width:68px;margin-left:auto;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-base);border:1px solid var(--dsw-alias-border-l2);border-radius:6px;padding:1px 6px;font:inherit;font-size:12px;line-height:18px;font-variant-numeric:tabular-nums;text-align:right;appearance:textfield}",
			".sco_mergeThresholdInput::-webkit-outer-spin-button,.sco_mergeThresholdInput::-webkit-inner-spin-button{appearance:none;margin:0}",
			".sco_mergeThresholdInput:focus{border-color:var(--dsw-alias-brand-primary);outline:none}",
			".sco_mergeThresholdUnit{color:var(--dsw-alias-label-tertiary)}"
		].join("");
		const tagId = "@kidli1412/dsh-session-cost/SessionCost.module.css";
		if (typeof document !== "undefined") {
			// The tag is reused across module rebuilds, so its CONTENT is
			// reconciled, not just its presence: an HMR-reloaded bundle would
			// otherwise keep serving the previous version's stylesheet until the
			// page was reloaded (and a new rule would silently not exist).
		...
		const S = {
			...
			mergePanelSetting: "sco_mergePanelSetting",
			mergePanelSettingLabel: "sco_mergePanelSettingLabel",
			mergeThresholdInput: "sco_mergeThresholdInput",
			mergeThresholdUnit: "sco_mergeThresholdUnit"
		};
		//#endregion
		 ...
		 * settings scope — NOT localStorage. The merge component reads the store
		 * through useSyncExternalStore and its panel's threshold field writes it
		 * back; while the scope is loading or unavailable the store falls back
		 * to the default (10 CNY), so the UI never blocks on the settings
		 * transport. `lowBalanceThreshold` (CNY, default 10) colors the balance
		 ...
		 * hand-written here (the cost pill's wallet, the refresh arrow, the
		 * panel head icon): the shipped icon module is not part of the client
		 * module graph at 0.1.5, and requiring it would throw inside the factory
		 * and take the whole client half down.
		...
		function updateMergeNode(previous, node) {
			...
			if (previousPanel !== null && nextPanel !== null) {
				...
				if (previousList !== null && nextList !== null) {
					// Each child of the list IS a slot — `dt` carries row-label,
					// `dd` carries row-value (or panel-balance) — while
					// `querySelector` only looks at DESCENDANTS. Resolving
					// `current.querySelector("[data-slot=row-label]")` therefore
					// returned null for every cell and `patchText` bailed out, so
					// no list row ever moved: a manual refresh updated 账户余额
					// (patched by slot name further up) while 充值/赠送余额 and
					// the per-model readings stayed frozen. Patch the cells.
					//
					// The threshold row is not in this list at all (it sits under
					// the 更新于/⟳ line and carries no patched slot), so a 30 s
					// refresh cannot eat a value the user is halfway through
					// typing.
					//
					// Snapshot the candidate's cells first: appending or
					// replacing MOVES a cell out of `nextList`, which would
					// shift the indices under us.
					const nextCells = Array.from(nextList.children);
					for (let index = 0; index < nextCells.length; index += 1) {
						const next = nextCells[index];
						const current = previousList.children[index];
						if (current === void 0) {
							// The list grew in place (the first model row
							// replacing 暂无 token 用量, the balance breakdown
							// arriving): take the freshly built cell.
							previousList.appendChild(next);
							continue;
						}
						if (current.getAttribute("data-slot") !== next.getAttribute("data-slot")) {
							// Same slot count but a different cell at this index:
							// a silently stale cell would be invisible, so take
							// the fresh one.
							current.replaceWith(next);
							continue;
						}
						patchText(current, next);
						patchWarn(current, next);
					}
					while (previousList.children.length > nextCells.length) previousList.lastElementChild.remove();
				}
		...
		function buildMergeNode(dict, data) {
			...
			// ---- the one setting: where does "low balance" start? ------------
			// A row of its own UNDER the 更新于/⟳ line, so the reading grid above
			// stays nothing but numbers and the panel's controls sit together.
			// It lives in the panel at all — rather than in 设置 → 插件 →
			// 插件配置 — because the threshold only exists to color this panel
			// and this pill: the place that shows the color owns the number.
			// The row is outside `panel-list` and carries no patched slot, so the
			// in-place update never touches it and a 30 s refresh cannot write
			// over a half-typed value.
			const setting = document.createElement("div");
			setting.className = S.mergePanelSetting;
			setting.setAttribute("data-slot", "panel-setting");
			const settingLabel = document.createElement("span");
			settingLabel.className = S.mergePanelSettingLabel;
			settingLabel.textContent = dict("settingsThreshold");
			setting.appendChild(settingLabel);
			const thresholdInput = document.createElement("input");
			thresholdInput.type = "number";
			thresholdInput.min = "0";
			thresholdInput.step = "1";
			thresholdInput.className = S.mergeThresholdInput;
			thresholdInput.setAttribute("data-slot", "threshold-input");
			thresholdInput.setAttribute("aria-label", dict("settingsThreshold"));
			thresholdInput.title = dict("settingsThresholdHint");
			const storedThreshold = data.lowBalanceThreshold ?? DEFAULT_LOW_BALANCE_CNY;
			thresholdInput.value = String(storedThreshold);
			/**
			 * Commit the field: an emptied or nonsensical box snaps back to the
			 * stored value (never a silent 0), a valid one is sanitized, echoed
			 * back, and handed to the owner. The handler is read at EVENT time
			 * like the ⟳ button: this node outlives many renders, so `data` is
			 * the stable state object whose fields the owner refreshes.
			 */
			const commitThreshold = () => {
				const text = String(thresholdInput.value ?? "").trim();
				const parsed = text === "" ? Number.NaN : Number(text);
				if (!Number.isFinite(parsed) || parsed < 0) {
					thresholdInput.value = String(data.lowBalanceThreshold ?? DEFAULT_LOW_BALANCE_CNY);
					return;
				}
				const next = sanitizeThreshold(parsed);
				thresholdInput.value = String(next);
				if (typeof data.onThreshold === "function") data.onThreshold(next);
			};
			// `change` covers blur and Enter; the explicit Enter listener keeps
			// the commit deterministic across browsers.
			...
			setting.appendChild(thresholdInput);
			const thresholdUnit = document.createElement("span");
			thresholdUnit.className = S.mergeThresholdUnit;
			thresholdUnit.textContent = dict("settingsThresholdUnit");
			setting.appendChild(thresholdUnit);
		...
		function SessionCostMerge({ useSession, useProjection, t }) {
			...
			// Low-balance threshold, owned by the panel's own field (it used to
			// be a card in 设置 → 插件 → 插件配置).
			...
			// The panel's threshold field writes straight into the settings
			// scope (the same `session-cost` namespace as before — the value is
			// durable, it just has a new editor). Reading `configStore` here
			// rather than capturing a value keeps the write correct across
			// renders, and the field's own change handler reads this function at
			// event time.
			input.onThreshold = (value) => {
				configStore.set({ lowBalanceThreshold: sanitizeThreshold(value) });
			};
		...
		//#region plugin body
		/**
		 * Settings namespace this plugin reads and writes — must match the
		...
		/**
		 * Client plugin body: register the dictionaries and the composer-dock
		 * entry — the hidden anchor whose observer appends the cost/balance
		 * segments into the built-in stats bar, and whose click-open panel
		 * carries the low-balance threshold field. Everything is backed by the
		 * `session-cost` settings namespace through a bound settings scope.
		 *
		 * There is deliberately NO `settings.plugin.item` registration: the
		 * threshold used to live in 设置 → 插件 → 插件配置, but it only exists
		 * to color this panel and this pill, so it moved into the panel itself.
		 * The namespace stays registered on the Host side, which keeps the
		 * value durable in settings.yaml and the settings scope bindable.
