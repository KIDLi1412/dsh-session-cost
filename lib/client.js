...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".sco_mergeVal{color:var(--dsw-alias-label-secondary);font-weight:600}",
		...
		/**
		 * The peak/off-peak/legacy cost split for one model, as a trailing
		 * `· 高峰 ¥x · 空闲 ¥y` suffix. Only rendered when a session actually
		 * spans more than one billing period (峰谷), so it stays out of the way
		 * in the common case.
		...
		/** The session-cost pill glyph: a ¥ badge (mirrors the built-in pills' leading icon). */
		function costIconSvg() {
			return svgEl({
				...
				children: [
					...
					["text", {
						x: "8", y: "11.4", "text-anchor": "middle", "font-size": "8.5",
						"font-family": "inherit", fill: "currentColor", text: "¥"
					}]
				]
			});
		}
		...
		/** The panel's title glyph: the same ¥ badge at title scale. */
		 ...
		 * The hard part is that the bar APPEARS LATE: since DSH 0.1.5
		 * `StatsPills` returns `null` until the session has a step or token
		 * activity, so the anchor's parent already exists while the bar does
		 * not. Observing only that parent's `childList` never fires when the bar
		 * finally mounts (the parent itself is untouched), which silently loses
		 * the merge — the observer therefore watches the whole subtree and
		 * re-runs `sync()` on any mutation in it.
		 *
		 * `writeMerge` returns the node to append (or null to remove); it is
		 * re-invoked on every sync so the latest cost/balance state and the
		 * current dictionary are always what lands in the DOM.
		 *
		 ...
		 * @returns a disposer that removes the observer, the merge node and the
		 *   inline styles applied to the bar.
		 * @internal
		 */
		function startStatsRowObserver(anchor, writeMerge) {
			if (anchor === null || anchor === void 0 || anchor.nodeType !== 1) return () => {};
			// Nothing to show at all (no cost yet, no balance yet): there is no
			// reason to keep observing the composer for a bar to decorate.
			if (writeMerge() === null) return () => {};
			let row = null;
			let observer = null;
			...
			const sync = () => {
				const found = findStatsRow(container());
				if (found === null) {
				...
				// Same shape: patch the value slots in place so the mounted
				// listeners (and an open panel) survive a data update — the
				// candidate node is then discarded, panel included.
			...
			if (typeof MutationObserver !== "undefined") {
				observer = new MutationObserver(() => {
					...
					try {
						...
						if (host !== null && host.isConnected) observer.observe(host, { childList: true, characterData: true, subtree: true });
					}
				});
				const host = container();
				if (host !== null) observer.observe(host, { childList: true, characterData: true, subtree: true });
			}
			return detach;
		}
		//#endregion
		...
		function updateMergeNode(previous, node) {
			...
			patchText(previous.querySelector("[data-slot=title-value]"), node.querySelector("[data-slot=title-value]"));
			patchText(previous.querySelector("[data-slot=panel-balance]"), node.querySelector("[data-slot=panel-balance]"));
			patchWarn(previous.querySelector("[data-slot=panel-balance]"), node.querySelector("[data-slot=panel-balance]"));
			patchText(previous.querySelector("[data-slot=panel-updated]"), node.querySelector("[data-slot=panel-updated]"));
			patchIcon(previous.querySelector("[data-slot=refresh-icon]"), node.querySelector("[data-slot=refresh-icon]"));
			patchText(previous.querySelector("[data-slot=panel-note]"), node.querySelector("[data-slot=panel-note]"));
			patchIcon(previous.querySelector("[data-slot=panel-head-icon]"), node.querySelector("[data-slot=panel-head-icon]"));
			const previousPanel = previous.querySelector("[data-slot=panel]");
			const nextPanel = node.querySelector("[data-slot=panel]");
			const previousTrigger = previous.querySelector("[data-slot=trigger]");
			if (previousPanel !== null && nextPanel !== null && previousTrigger !== null) {
				...
				// `placeOpenPanel` after this patch, never copied from the
				// freshly built node (which was never laid out).
				if (!wantOpen) previousPanel.style.visibility = "hidden";
			}
			const previousRefresh = previous.querySelector("[data-slot=refresh]");
			const nextRefresh = node.querySelector("[data-slot=refresh]");
			...
			const previousList = previous.querySelector("[data-slot=panel-list]");
			const nextList = node.querySelector("[data-slot=panel-list]");
		 ...
		 * pair for the shared stat-dialog grid.
		 * @param dict - localized dictionary.
		 * @param row - one per-model summary row.
		 */
		function modelRowElement(dict, row) {
			...
			dd.textContent = `${dict("tooltipInput")} ${fmt(row.inputTokens)} · ${dict("tooltipOutput")} ${fmt(row.outputTokens)} · ${dict("currencySymbol")}${fmtCny(row.cost, 4)}${billingSplitSuffix(dict, row)}`;
			return [dt, dd];
		}
		...
		function disposeMergeNode(node) {
			...
			const panel = mergePanels === null ? null : mergePanels.get(node);
			if (mergePanels !== null) mergePanels.delete(node);
			node.dispatchEvent(new Event("session-cost:dispose"));
			if (panel !== null && panel !== void 0 && panel.isConnected) panel.remove();
		}
		...
		function placeOpenPanel(node) {
			...
			if (panel === null || panel === void 0 || trigger === null || panel.hidden === true) return;
		...
		function buildMergeNode(dict, data) {
			...
			titleValue.textContent = costError === null
				? `${dict("currencySymbol")}${fmtCny(cost, 4)}`
				: dict("unavailable");
			...
			if (models.length === 0) {
				details.appendChild(rowElement(dict("tooltipNoModels"), ""));
			} else {
			...
			if (typeof onRefresh === "function") refresh.addEventListener("click", onRefresh);
		...
		/** The `本会话费用 ¥x.xxxx` segment of the pill label. */
		function costSegment(dict, cost, costError) {
			...
			value.textContent = `${dict("cost")} ${dict("currencySymbol")}${fmtCny(cost, 4)}`;
			return value;
		}
		...
		function buildBalanceSegment(dict, data) {
			...
			// The two readings are one pill sentence, so they are joined by the
			// same middot the built-in pills use between their own figures.
			...
			if (balanceError !== null) {
				...
				value.textContent = `${dict("balance")} ${dict("currencySymbol")}${fmtCny(data.totalValue, 2)}`;
			}
		...
		/**
		 * Merge-row CSS applied while the merge is attached. Both built-in
		 * shapes cap themselves at `--dsh-chat-content-width` (748 px): the
		 * ≤ 0.1.4 `StatsLine` root also carried `overflow:hidden` +
		 * `text-overflow:ellipsis` (DSH rc.7+), which clipped the cost/balance
		 * segment appended at the row's tail so the merge silently disappeared,
		 * and the 0.1.5 `StatsPills` root is a centered flex row whose shrink
		 * math the appended item would otherwise join. Widening the row
		 * (`max-width:none`) and making overflow visible shows the merge
		 * without relying on the zh_pro "统计全显示" tweak doing the same thing.
		 * Only INLINE styles are touched and the previous inline values are
		 * restored on teardown, so DSH's own stylesheet (and zh_pro's
		 * !important overrides) still win.
		 * @internal
		 */
		...
		function SessionCostMerge({ useSession, useProjection, t }) {
			...
			// The setter's identity is stable, so the merge effect can depend on
			// `panelOpen` alone.
			...
			// and why the observer must span a subtree).
			//
			// The bar caps at the 748 px content width with `overflow:hidden` on
			// the ≤ 0.1.4 text row, which clips an appended tail;
			// `applyMergeRowStyles` (inside the observer helper) widens +
			// unclips it so the merge stays visible even without the zh_pro
			// "统计全显示" tweak, and the prior inline values are restored on
			// teardown.
			react.useEffect(() => startStatsRowObserver(anchorRef.current, () => buildMergeNode(dictRef.current, {
				cost,
				models,
				balance: bal,
				totalValue,
				...
				summary
			})), [lowBalanceThreshold, summary, balance, summaryError, balanceError, refreshing, justRefreshed, cost, models, totalValue, hasBalance, bal, panelOpen, togglePanel]);
