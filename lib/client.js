...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			// The panel's one setting: a right-aligned number field that sits in
			// the dd column (tabular digits, 12/18) with DSH's own input recipe
			// (bg-base on a border-l2 stroke, rounded 6). The native spinners are
			// hidden — at this width they would overlap the number — while the
			// field stays a number input, so ↑/↓ still steps it.
			".sco_mergePanelDetails dd[data-slot=threshold]{justify-content:flex-end;align-items:center;gap:4px;display:flex}",
			".sco_mergeThresholdInput{box-sizing:border-box;width:68px;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-base);border:1px solid var(--dsw-alias-border-l2);border-radius:6px;padding:1px 6px;font:inherit;font-size:12px;line-height:18px;font-variant-numeric:tabular-nums;text-align:right;appearance:textfield}",
		...
		function updateMergeNode(previous, node) {
			...
			if (previousPanel !== null && nextPanel !== null) {
				...
				if (previousList !== null && nextList !== null) {
					...
					//
					// The threshold row rides along untouched: its `dd` holds an
					// input, so `patchText` skips it (a cell with slots is never
					// written as text) and a 30 s refresh cannot eat a value the
		...
		function buildMergeNode(dict, data) {
			...
			// It lives HERE rather than in 设置 → 插件 → 插件配置: the threshold
			// only exists to color this panel and this pill, so the place that
			// shows the color is the place that should own the number. The cell
			// holds an `<input>` (a slot of its own), which is what keeps the
			// in-place patch from ever writing over a half-typed value — see the
			// list patch in `updateMergeNode`.
			const thresholdLabel = document.createElement("dt");
			thresholdLabel.setAttribute("data-slot", "row-label");
			thresholdLabel.textContent = dict("settingsThreshold");
			const thresholdCell = document.createElement("dd");
			thresholdCell.setAttribute("data-slot", "threshold");
			...
			thresholdCell.appendChild(thresholdUnit);
			details.appendChild(thresholdLabel);
			details.appendChild(thresholdCell);
