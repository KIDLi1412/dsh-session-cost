...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			// the pill label's ellipsis clamp; the value tier mirrors the pills'
			// own 13/20 metrics, with the ship's label-secondary on hover.
			...
			".sco_merge{display:inline-flex;align-items:center;gap:6px;max-width:none;flex:none;vertical-align:baseline;white-space:nowrap;color:var(--dsw-alias-label-tertiary)}",
			".sco_merge:hover{color:var(--dsw-alias-label-secondary)}",
			".sco_merge > svg{width:14px;height:14px;flex:none}",
			...
			".sco_mergeBtn{cursor:pointer;color:var(--dsw-alias-label-tertiary);background:0 0;border:none;border-radius:24px;width:16px;height:20px;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex;vertical-align:middle;font-size:10px;line-height:1}",
			".sco_mergeBtn:hover{color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-interactive-bg-hover)}",
		...
		/** Create the inline "session cost" glyph (a ¥ badge) shown beside the cost reading. */
		function costIconSvg() {
			...
			glyph.setAttribute("x", "8");
			glyph.setAttribute("y", "11.4");
		...
		function startStatsRowObserver(anchor, writeMerge) {
			...
			const sync = () => {
				...
				// Identical content: keep the existing node (its click listener
				// stays wired); only rebuild when the data actually changed.
		...
		/**
		 * Build the DOM node appended into the built-in stats bar: the cost
		 * reading `[¥] 本会话费用 ¥0.4549` followed by `| 余额 ¥7.09` and the
		 * ⟳ refresh button, with a hover title carrying the per-model
		 * breakdown. Since DSH 0.1.5 the bar is a centered flex row of icon
		 * pills, so this node is built as one more flex item in the same
		 * 13/20 secondary tier (glyph first, then label, then value); the
		 * leading pill separator is only added when the bar itself has no
		 * rendering yet (nothing to separate from). `dict` provides localized
		 * labels, `data` carries the current cost/balance state, `onRefresh`
		 * wires the button.
		...
		function buildMergeNode(dict, data) {
			...
			if (costError === null) {
				addSep();
				container.appendChild(costIconSvg());
